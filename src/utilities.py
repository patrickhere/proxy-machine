...
def parse_crop_string(
    crop_string: Optional[str], card_width: int, card_height: int
) -> Tuple[float, float]:
    """
...
def get_back_card_image_path(back_dir_path) -> Optional[str]:
    # List all files in the directory that do not end with .md
    # The directory may contain markdown files
    files = [
        f
        for f in os.listdir(back_dir_path)
        if (os.path.isfile(os.path.join(back_dir_path, f)) and not f.endswith(".md"))
    ]
...
def draw_card_with_bleed(
    ...
    extend_edge(
        (0, height - 1, width, height), (origin_x, origin_y + height), y_bleed, Axis.Y
    )

    # Left and right
    extend_edge((0, 0, 1, height), (origin_x - x_bleed, origin_y), x_bleed, Axis.X)
    extend_edge(
        (width - 1, 0, width, height), (origin_x + width, origin_y), x_bleed, Axis.X
    )

    # Corners
    for x_bleed_val, crop_x, pos_x in [
        (x_bleed, 0, origin_x - x_bleed),
        (x_bleed, width - 1, origin_x + width),
    ]:
        for y_bleed_val, crop_y, pos_y in [
            (y_bleed, 0, origin_y - y_bleed),
            (y_bleed, height - 1, origin_y + height),
        ]:
            for x_bleed_i in range(x_bleed_val):
                for y_bleed_i in range(y_bleed_val):
                    base_image.paste(
...
def draw_card_layout(
    card_images: List[Optional[Image.Image]],
    base_image: Image.Image,
    num_rows: int,
    num_cols: int,
    x_pos: Sequence[int],
    ...
    for i, card_image in enumerate(card_images):
        ...
        if flip:
            new_origin_y = math.floor(
                y_pos[num_rows - ((i % num_cards) // num_cols) - 1] * ppi_ratio
            )
        ...
        card_image = card_image.resize(
            (math.floor(width * ppi_ratio), math.floor(height * ppi_ratio))
        )
        ...
        card_image = card_image.crop(
            (
                extend_corners_ppi,
                extend_corners_ppi,
                card_image.width - extend_corners_ppi,
                card_image.height - extend_corners_ppi,
            )
        )

        adjusted_print_bleed: tuple[int, int] = (
            math.ceil(print_bleed[0] * ppi_ratio) + extend_corners_ppi,
            math.ceil(print_bleed[1] * ppi_ratio) + extend_corners_ppi,
        )
...
def add_front_back_pages(
    ...
    draw.text(
        (
            math.floor((page_width - 180) * ppi_ratio),
            math.floor((page_height - 140) * ppi_ratio),
        ),
        label,
        fill=(0, 0, 0),
        anchor="ra",
...
def generate_pdf(
    ...
    if only_fronts:
        if len(ds_set) > 0:
            raise Exception(
                f'Cannot use "--only_fronts" with double-sided cards. Remove cards from double-side image directory "{double_sided_dir_path}".'
            )
    ...
    with open(layouts_path, "r") as layouts_file:
        ...
        if card_size not in layouts.card_sizes:
            raise Exception(
                f'Unsupported card size "{card_size}". Try card sizes: {paper_layout.card_layouts.keys()}.'
            )
        ...
        if card_size not in paper_layout.card_layouts:
            raise Exception(
                f'Unsupported card size "{card_size}" with paper size "{paper_size}". Try card sizes: {paper_layout.card_layouts.keys()}.'
            )
        card_layout = paper_layout.card_layouts[card_size]

        # Determine the amount of x and y crop
        crop = parse_crop_string(
            crop_string, card_layout_size.width, card_layout_size.height
        )
        ...
        with Image.open(registration_path) as reg_im:
            ...
            if not use_default_back_page and back_card_image_path:
                # Load the card back image
                with Image.open(back_card_image_path) as back_im:
                    back_im = ImageOps.exif_transpose(back_im)

                    back_images: List[Optional[Image.Image]] = [back_im] * num_cards
            ...
            while True:
                file_group = list(
                    itertools.islice(it, num_cards - len(clean_skip_indices))
                )
                if not file_group:
                    break

                # Fetch card art
                front_card_images: List[Optional[Image.Image]] = []
            ...
            while True:
                ...
                # Fetch card art
                front_card_images = []
                back_card_images: List[Optional[Image.Image]] = []
                file_group_iterator = iter(file_group)
                for i in range(num_cards):
                    if i in clean_skip_indices:
                        front_card_images.append(None)
            ...
            if load_offset:
                ...
                if saved_offset is None:
                    ...
                    print(
                        f"Loaded x offset: {saved_offset.x_offset}, y offset: {saved_offset.y_offset}"
                    )
                    pages = offset_images(
                        pages, saved_offset.x_offset, saved_offset.y_offset, ppi
                    )
...
def save_offset(x_offset: int, y_offset: int) -> None:
    ...
    with open("data/offset_data.json", "w") as offset_file:
        offset_file.write(
            OffsetData(x_offset=x_offset, y_offset=y_offset).model_dump_json(indent=4)
        )

    print("Offset data saved!")


def load_saved_offset() -> Optional[OffsetData]:
...
def offset_images(
    images: List[Image.Image], x_offset: int, y_offset: int, ppi: int
) -> List[Image.Image]:
    offset_images = []
...
def calculate_max_print_bleed(
    x_pos: List[int], y_pos: List[int], width: int, height: int
) -> tuple[int, int]:
