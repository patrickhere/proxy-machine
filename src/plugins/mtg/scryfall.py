...
double_sided_layouts: Set[str] = {"transform", "modal_dfc"}


def request_scryfall(query: str) -> requests.Response:
    r = requests.get(
    ...
    if layout in double_sided_layouts:
        ...
        if card_art is not None:
            ...
            for counter in range(quantity):
                image_path = os.path.join(
                    double_sided_dir,
                    f"{str(index)}{clean_card_name}{str(counter + 1)}.png",
                )
...
def partition_printings(
    printings: Sequence[Any], condition: Callable[[Any], bool]
...
def progressive_filtering(
    printings: Sequence[Any], filters: Iterable[Callable[[Any], bool]]
) -> List[Any]:
    pool: List[Any] = list(printings)
    leftovers: List[Any] = []
...
def fetch_card(
    ...
    if (
        ...
        and card_set != ""
        and card_collector_number != ""
    ):
        card_info_query = (
            f"https://api.scryfall.com/cards/{card_set}/{card_collector_number}"
        )
        ...
        card_info_query = (
            f"https://api.scryfall.com/cards/named?exact={clear_card_name}"
        )

        # Query for card info
        card_json = request_scryfall(card_info_query).json()

        card_set_resolved = card_json["set"]
        collector_number_resolved = card_json["collector_number"]

        # If preferred options are used, then filter over prints
        if (
            prefer_older_sets
            or len(preferred_sets) > 0
            or prefer_showcase
            or prefer_extra_art
        ):
            # Get available printings
            prints_search_json = request_scryfall(card_json["prints_search_uri"]).json()
            card_printings: List[Any] = prints_search_json.get("data", [])
            ...
            filters = [
                ...
                lambda c: not prefer_showcase
                ^ ("frame_effects" in c and "showcase" in c["frame_effects"]),
                lambda c: not prefer_extra_art
                ^ (
                    c["full_art"]
                    or c["border_color"] == "borderless"
                    or ("frame_effects" in c and "extendedart" in c["frame_effects"])
                ),
            ]
            ...
            if len(filtered_printings) == 0:
                ...
                card_set_resolved = best_print.get("set", card_set_resolved)
                collector_number_resolved = best_print.get(
                    "collector_number", collector_number_resolved
                )
...
def get_handle_card(
    ...
    def configured_fetch_card(
        ...
        card_collector_number: Optional[str] = None,
        quantity: int = 1,
    ):
        fetch_card(
            index,
