...
RIFTMANA_URL_TEMPLATE = (
    "https://riftmana.com/wp-content/uploads/Cards/{card_number}.webp"
)
...
def fetch_card_art(
    index: int, card_number: str, quantity: int, source: ImageServer, front_img_dir: str
):
...
def fetch_card_number(name: str) -> str:
    ...
    card_link = (
        name_response.json()[0].get("_links", {}).get("wp:post_type")[0].get("href")
    )
    ...
    raise ValueError(f"Unable to resolve card number for '{name}'.")


def get_handle_card(source: ImageServer, front_img_dir: str):
    def configured_fetch_card(index: int, card_number: str, quantity: int):
        fetch_card_art(index, card_number, quantity, source, front_img_dir)
